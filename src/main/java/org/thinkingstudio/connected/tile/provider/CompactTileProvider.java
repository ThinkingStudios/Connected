...
public class CompactTileProvider implements TileProvider {
	...
	public CompactTileProvider(Tile[] origTiles) throws IOException {
		...
		for(int i = 0; i <= 46; i++) {
			...
			short bithack = Connected.CTM_TO_IDEALIZED_BITHACK[i];
			...
			spriteIds.add(new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, Connected.resourcePack.dynamicallyPutImage(tile)));
		}
	}
